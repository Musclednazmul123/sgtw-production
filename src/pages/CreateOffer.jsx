...
export default function CreateOffer({ method }) {
    ...
    const handleTriggerValue = useCallback(
        (check, trigger_type) => {
    ...
    const TagInputForm = useCallback(() => {
        ...
        if (
            tag_input &&
            tag_input.trim() &&
            t.find((key) => key.toUpperCase() === tag_input.toUpperCase()) ==
                undefined &&
            !trigger_tags.includes(tag_input.trim().replace(/\,/g, ""))
        ) {
    ...
    const handleClick = () => {
        clicked ? setClicked("") : setClicked("click-state");
    ...
    function disableSaveButton() {
        var disabled = false;
        if (!upsell_in_cart && !upsell_product_page) disabled = true;
        if (!offer_name || offer_name.trim() === "" || offer_name.length < 2)
            disabled = true;
        if (trigger_type == "specific_product" && trigger_products.length < 1)
            disabled = true;
        if (trigger_type == "tagged_product" && trigger_tags.length < 1)
            disabled = true;
    ...
    function createOfferAPI() {
        if (method === "edit") {
            ...
            Axios(
                {
                    type: "put",
                    url: "/merchant/update_offer?id=" + id,
    ...
    return (
        <Page>
            <TitleBar
                ...
                title={method === "edit" ? offer_name : "Add Offer"}
                primaryAction={{
                    content: "Save offer",
                                ...
                                <Card.Section>
                                    <Stack vertical>
                                        ...
                                        <TextStyle variation="subdued">
                                            ...
                                            along with the burger, French Fries
                                            will be the offer product.
                                        </TextStyle>
