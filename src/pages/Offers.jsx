import { Page, Layout, Card, Stack, TextContainer, TextStyle, Heading, Badge, TextField, Icon, Select, DataTable, Button, Pagination, ButtonGroup, Modal, EmptyState, Popover, ActionList } from "@shopify/polaris";
...
import {
  EditMajor, DeleteMajor, ViewMajor, ImportMinor, ExportMinor
...
function st(params) {
  Axios({
    type: "get",
    url: "/merchant/shop_summary?shop="+store("shop"),
    headers: {
  ...
  function DeleteOffers(){
    ...
    Axios({
      type: "delete",
      url: "/merchant/offers/delete?id="+delete_offer_id+"&shop="+store("shop"),
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    return(
      ...
      />
        <Layout>
          ...
          >
            <p>Start adding Upsell offers which loves by your customers.</p>
          </EmptyState> 
        </Card>
          </Layout.Section>
        </Layout>
    ...
    <DataTable
      rows={rows.map((row) => [
        <div>
          <Toggle
            ...
            onChange={(e) => {
              ...
              Axios({
                url: `/merchant/offer/status?id=${row._id}`,
                type: "put",
                data: {
                  active: active,
                  id: row._id,
        ...
        <ButtonGroup segmented>
          {
            row.trigger_type == "specific_product" && 
        ...
        <Button
            icon={AnalyticsMajor}
            onClick={() => {
                navigate("/offers/show/"+row.uid);
                replace({pathname: '/offer/stats'});
            }}
          />
          <Button
            icon={EditMajor}
            onClick={() => {
              if (typeof onEditClick === "function") {
                navigate("/offers/edit/"+row._id);
                replace({pathname: '/offer/edit/'+row._id});
              }
            }}
          />
          <Button
