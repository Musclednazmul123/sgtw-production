...
window.UPSELL = (function () {
   ...
   function addToCart(el, items) {
       ...
       $SH.ajax({
           ...
           error: function (error) {
               log("product Not Addedd !");
               $SH('.isf-frequently-bought-add-button').after("<div class='soldout_error'>Products are sold out !</div>");
               $SH('.soldout_error').remove();
   ...
   function isTriggerByProducts(arr, product_id) {
       // // debugger;
   ...
   function isTriggerByTags(arr1, arr2) {
       var _isTriggered = false;
   ...
   function getMatchingProducts(params, callback) {
       ...
       var active_offer = {};
       var array_r = [];
       if (params.upsell && UpsellLiquid && UpsellLiquid.Offers && UpsellLiquid.Offers.length > 0) {
           array_r = UpsellLiquid.Offers;
       }
       else if (UpsellLiquid && UpsellLiquid.upsell_frequently && UpsellLiquid.upsell_frequently.length > 0) {
           array_r = UpsellLiquid.upsell_frequently;
       }
       for (var index = 0, leng = array_r.length; index < leng; index++) {
           var offer = array_r[index];
           var isThisTemplate = getTemplate(template,
           {
               upsell_in_cart: offer.upsell_in_cart,
               upsell_product_page: offer.upsell_product_page,
           });
           if (isThisTemplate) {
               if(offer.trigger_type === "all_products"){
                   active_offer = offer;
               }
               else if(offer.trigger_type === "specific_product"){
                   if (isTriggerByProducts(offer.trigger_products, product_id)) {
                       active_offer = offer;
                       break;
                   }
               }
               else if(offer.trigger_type === "tagged_product"){
                   if (isTriggerByTags(offer.trigger_tags, product_tags)) {
                       active_offer = offer;
                       break;
                   }
               }
   ...
   function getMatchingProductForFBT(params, callback) {
       // debugger;
       if (!params) {
           return null;
       }
   ...
   function getProductByHandle(handle, callback) {
       if (handle) {
           $SH.getJSON(Shopify.routes.root + "products/" + handle + ".js", function(product, error){
               if (product) {
                   if (typeof callback === "function") {
   ...
   function UpsellWidget() {
       ...
       UPSELL.getMatchingProducts({
           ...
           console.log('upsell_response',upsell_response);
           var count = 0;
   ...
   function FBTWidget() {
       // debugger;
       UPSELL.getMatchingProductForFBT({
           product_id: UpsellLiquid.product_id,
           product_handle: UpsellLiquid.product_handle,
           template: UpsellLiquid.template
       }, function (response) {
   ...
   var $_slider = {
       ...
       infinite: true,
       speed: 400,
       slidesToShow: UpsellLiquid.recently_columns_desktop,
       slidesToScroll: 5,
       swipeToSlide: true,
   ...
   function render_recently_products(products) {
       products.forEach((product, index) => {
           ...
           price += variant.price;
           var recently = {};
           if (UpsellLiquid && UpsellLiquid.upsell_recently) {
               try {
                   recently = UpsellLiquid.upsell_recently[0];
               } catch (e) {}
           }
                   ...
                   function _this(matching_product) {
                       UPSELL.getProductByHandle(matching_product.handle, function (product, error) {
                           ...
                           if (response.active_offer.matching_products[count]) {
                             ...
                             console.log(render_products_data);
                               render_frequently_products(render_products_data);
                               if (typeof global_callback === "function") {
                                   global_callback("FBT Cart");
                               }
                           }
                       });
                   }
   ...
   function attachEvents() {
       ...
       if(UpsellLiquid.frequently_modal && modal && UpsellLiquid.template == 'cart'){
       ...
       $SH(".upsell_products .upsell_item").on('click', function () {
           var _this = $SH(this);
       ...
       $SH(".upsell_btn").on("click", function () {
           var vid = $SH(this).attr('data-id');
           var item = [{
               id: vid,
               quantity: 1
           }];
       ...
       $SH(".isf-frequently-bought-add-button").on("click", function () {
           var items = [];
       ...
       $SH('.isf-frequently-bought-selector-list input[type=checkbox]').on("change", function () {
       ...
       $SH('select[name="id"]').on('change', function () {
           ...
           var data_image = $SH(this).find('[selected="selected"]').attr('data-image');
           var this_handle = $SH(this).parents('li').attr('product-handle');
   ...
   function global_callback(type){
       $SH(".upsell-main-block").show();
       // events attach
       attachEvents();
   }
   ...
   return {
       ...
       getMatchingProductForFBT: getMatchingProductForFBT,
